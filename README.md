...
More information about the archiving project can be found on the ArchiveTeam wiki: [BBoard.de](http://archiveteam.org/index.php?title=BBoard.de)
...
Follow the [instructions on the ArchiveTeam wiki](http://archiveteam.org/index.php?title=Warrior) for installing the Warrior, and select the "BBoard.de" project in the Warrior interface.
...
after which the project can be run:
    ...
    docker run --name archiveteam --label=com.centurylinklabs.watchtower.enable=true --restart=unless-stopped atdr.meo.ws/archiveteam/bboard-grab --concurrent 1 YOURNICKHERE
...
Package `libzstd-dev` version 1.4.4 is required which is currently available from `buster-backports`.
    ...
    python3 -m pip install setuptools wheel
    python3 -m pip install --upgrade seesaw zstandard requests
    su -c "cd /home/archiveteam; git clone https://github.com/ArchiveTeam/bboard-grab.git; cd bboard-grab; ./get-wget-lua.sh" archiveteam
    screen su -c "cd /home/archiveteam/bboard-grab/; run-pipeline3 pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam
    [... ctrl+A D to detach ...]
...
**There is a known issue with some packaged versions of rsync. If you get errors during the upload stage, bboard-grab will not work with your rsync version.**
...
6. `screen su -c "cd /home/archiveteam/bboard-grab/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam`
...
Close the repo and build Wget-AT:
    ...
    git clone https://github.com/ArchiveTeam/bboard-grab
    cd bboard-grab; ./get-wget-lua.sh
...
Have an issue not listed here? Join us on IRC and ask! We can be found at hackint IRC [#msgbored](https://webirc.hackint.org/#irc://irc.hackint.org/#msgbored).
