...
LUA_SHA1 = get_hash(os.path.join(CWD, 'bboard.lua'))
...
class WgetArgs(object):
    ...
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'forum':
                wget_args.extend(['--warc-header', 'bboard-forum: '+item_value])
                wget_args.append('https://{}.bboard.de/'.format(item_value))
            elif item_type == 'topic':
                forum_name, forum_id, topic_id = item_value.split(':', 2)
                wget_args.extend(['--warc-header', 'bboard-forum: '+item_value])
                wget_args.extend(['--warc-header', 'bboard-forum-id: '+forum_id])
                wget_args.extend(['--warc-header', 'bboard-forum-{}-topic: {}'.format(forum_name, topic_id)])
                wget_args.append('https://{}.bboard.de/board/ftopic-{}-{}.html'.format(forum_name, forum_id, topic_id))
            else:
...
project = Project(
    title='BBoard.de',
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/a/aa/Bboard-logo.jpg" height="50px" title=""/>
        <h2>bboard.de <span class="links"><a href="https://bboard.de/">Website</a> &middot; <a href="http://tracker.archiveteam.org/bboard/">Leaderboard</a></span></h2>
        <p>Archiving BBoard.de.</p>
    '''
)
